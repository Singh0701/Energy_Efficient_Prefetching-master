...
#include "prefetch_throttler.h"

#include<vector>

#include<iterator>

uint64_t last_prefetch_cycle = 0; 
...
//Neelu: Todo: This knob will bypass translations too.
...
#ifdef FDP_PREFETCH_THROTTLER
	if(warmup_complete[fill_cpu])
	{
	//Pollution filter updates
	if(block[set][way].valid && !block[set][way].prefetch && (block[set][way].used) && MSHR.entry[mshr_index].type == PREFETCH)
	{
		//This means a non-prefetch demand block is being evicted by a prefetch request
		uint64_t pollution_filter_index = (block[set][way].address & ((1 << 12)-1)) ^ ((block[set][way].address >> 12) & ((1 << 12)-1)); 
		assert(pollution_filter_index < 4096);
		pollution_filter[pollution_filter_index] = 1;  
	}

	if(MSHR.entry[mshr_index].type == PREFETCH)
	{
		uint64_t pollution_filter_index = (MSHR.entry[mshr_index].address & ((1 << 12)-1)) ^ ((MSHR.entry[mshr_index].address >> 12) & ((1 << 12)-1));
		pollution_filter[pollution_filter_index] = 0;
	}
	...
	{
		eviction_count++;
		if(eviction_count == ((NUM_SET*NUM_WAY)/2))
		{
			//compute the counters
			//prefetcher accuracy 
			float accuracy_counter = (float(prefetcher_accuracy_value_at_the_start_of_interval)/2) + (float(float(used_total)/pref_total)/2);
			pref_total=0;
			prefetcher_accuracy_value_at_the_start_of_interval = accuracy_counter;

			//prefetcher lateness
			float lateness_counter = (float(prefetcher_lateness_value_at_the_start_of_interval)/2) + (float(float(late_total)/used_total)/2);
			used_total = 0;
			late_total = 0;
			prefetcher_lateness_value_at_the_start_of_interval = lateness_counter; 

			//cache pollution
			float pollution_counter = (float(prefetcher_pollution_value_at_the_start_of_interval)/2) + (float(float(pollution_total)/demand_total_misses)/2);
			pollution_total=0;
			demand_total_misses=0;
			prefetcher_pollution_value_at_the_start_of_interval = pollution_counter;

			//call throttler
			dynamic_config_counter = fdp_prefetch_throttler_operate(accuracy_counter, lateness_counter, pollution_counter, dynamic_config_counter);

			//reset eviction_count
			eviction_count=0;
		}
	}
	}
...
#ifdef FDP_PREFETCH_FILTER
		if(warmup_complete[writeback_cpu])
		{
                demand_total_misses++;

                uint64_t pollution_filter_index = (WQ.entry[index].address & ((1 << 12)-1)) ^ ((WQ.entry[index].address >> 12) & ((1 << 12)-1));
                if(pollution_filter[pollution_filter_index])
                        pollution_total++;
...
void CACHE::handle_read()
{
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
      ...
      if (read_cpu == NUM_CPUS)
	    ...
	    //	    way = 1;
		    ...
		    {
			ooo_cpu[cpu].LQ.entry[RQ.entry[index].lq_index].l2_or_llc_hit = true;
		    }
	...
	/*	if(cache_type==0)	//perfect-ITLB and baseline DTLB
		...
		uint64_t pollution_filter_index = (RQ.entry[index].address & ((1 << 12)-1)) ^ ((RQ.entry[index].address >> 12) & ((1 << 12)-1)); 
		if(pollution_filter[pollution_filter_index])
			pollution_total++; 
		}
#endif
...
//		cout<<"Aye Aye, Captain, requested.";
    ...
    if (PQ.occupancy < PQ.SIZE) {
	    ...
	    if(last_prefetch_cycle != current_core_cycle[cpu])
		    prefetch_cycle_count++;

	    last_prefetch_cycle = current_core_cycle[cpu]; 
